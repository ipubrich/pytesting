...
rows = []
with open('your_spreadsheet.csv', 'r', encoding='utf-8', errors='ignore') as f:
    reader = csv.DictReader(f)
    next(reader)  # skip the column headers row
    for row in reader:
        rows.append(row)

# Calculate failure count and total checks
failures = [row for row in rows if row['Mandatory Control'] == '0' and row['Sub Control ID'] == '0' and row['Subcontrol'] != '']
failure_count = {}
for row in failures:
...
with open('output_spreadsheet.csv', 'w', newline='', encoding='utf-8') as f:
