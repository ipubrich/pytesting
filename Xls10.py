...
for row in rows:
    if row['Subcontrol'] != '':
        key = (row['Dashboard Family'], row['Mandatory Control'], row['Sub Control ID'], row['Rule Name'])
        failure = row['checkstatus'] == '0'
        count = failure_count.get(key, 0)
        failure_count[key] = count + failure
        count = total_checks.get(key, 0)
        total_checks[key] = count + int(row['checkstatus'])

# Write the output to a new CSV file
with open('output.csv', 'w', newline='', encoding='utf8') as f:
    writer = csv.writer(f)
    writer.writerow(['Dashboard Family', 'Mandatory Control', 'Sub Control ID', 'Rule Name', 'Failure Count', 'Total Checks'])
    for key in sorted(failure_count.keys()):
        dashboard_family, mandatory_control, sub_control_id, rule_name = key
        failure_count_val = failure_count.get(key, 0)
        total_checks_val = total_checks.get(key, 0)
        writer.writerow([dashboard_family, mandatory_control, sub_control_id, rule_name, failure_count_val, total_checks_val])
